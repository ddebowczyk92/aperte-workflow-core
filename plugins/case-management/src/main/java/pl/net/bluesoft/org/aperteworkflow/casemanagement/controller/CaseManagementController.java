package pl.net.bluesoft.org.aperteworkflow.casemanagement.controller;
...
import pl.net.bluesoft.org.aperteworkflow.casemanagement.controller.bean.CaseDTO;
...
import pl.net.bluesoft.org.aperteworkflow.casemanagement.model.CaseStateWidget;
import pl.net.bluesoft.org.aperteworkflow.casemanagement.processor.CaseProcessor;
import pl.net.bluesoft.org.aperteworkflow.casemanagement.ui.CaseViewBuilder;
...
public class CaseManagementController implements IOsgiWebController {
    ...
    @ControllerMethod(action = "findCase")
    public GenericResultBean findCase(final OsgiWebRequest invocation) {

        String caseNumberQuery = invocation.getRequest().getParameter(SEARCH_QUERY_REQ_PARAM_NAME);
        String caseType = invocation.getRequest().getParameter(CASE_TYPE_REQ_PARAM_NAME);
        Integer pageLimit = Integer.valueOf(invocation.getRequest().getParameter(PAGE_LIMIT_REQ_PARAM_NAME));
        Integer page = Integer.valueOf(invocation.getRequest().getParameter(PAGE_PARAM_NAME));

        GenericResultBean result = new GenericResultBean();
        try {
            List<CaseDTO> caseBeans = createDTOList(
                    facade.findCasesByNumberAndSimpleAttr(caseNumberQuery, "complaintType", caseType, pageLimit, page),
                    invocation.getProcessToolRequestContext().getMessageSource());
            Long totalCount = facade.getCasesByNumberAndSimpleAttrCount(caseNumberQuery, "complaintType", caseType);
            Collections.sort(caseBeans);
            final Select2PagingBean pagingBean = new Select2PagingBean(caseBeans, totalCount);
            return pagingBean;
        } catch (Exception e) {
            logger.log(Level.SEVERE, "[CASE_MANAGEMENT] Cannot get the case list", e);
            result.addError("Cannot get the case list", e.getMessage());
    ...
    protected Long getCaseId(final HttpServletRequest request) {
        ...
        Long caseId = param != null ? Long.valueOf(param) : null;
        if (caseId == null) {
    ...
    protected String buildCaseView(final Case caseInstance, final ProcessToolContext ctx, final IProcessToolRequestContext requestContext) {
        ...
        CaseViewBuilder viewBuilder = new CaseViewBuilder(caseInstance)
                .setWidgets(widgets)
